...
void UHF_RFID::ToHex(UDOUBLE parameters, UBYTE MSB, UBYTE LSB)
{
  UBYTE i = LSB - MSB;
  UBYTE l = LSB;
...
UBYTE UHF_RFID::Verify_the_return(UBYTE a[] , UBYTE size_v)  //Verify return information 验证返回信息
{
  ...
  Serial.print("Expected Response Pattern: ");
    for (UBYTE i = 0; i < size_v; i++) {
        Serial.print(a[i], HEX); // Print in hexadecimal format
        Serial.print(" ");
    }
    Serial.println();
    ...
    for (UBYTE i = 0; i < size_v; i++) {
        Serial.print(DATA_I[i], HEX); // Print in hexadecimal format
        Serial.print(" ");
    }
    Serial.println();
    

    if (size_v != sizeof(DATA_I)) {
    Serial.println("Received response size does not match the expected size.");
   
}

Serial.print("Expected Response Size: ");
Serial.println(size_v);
Serial.print("Received Response Size: ");
Serial.println(sizeof(DATA_I));
  ...
  if ( DATA_I[0] == a[0] && DATA_I[1] == a[1] )
  {
    ...
    Serial.print("Number of matching bytes: ");
Serial.println(c);
    if (c == size_v)
    {
      return 1;
