...
ManyInfo UHF_RFID::Multiple_polling_instructions(UWORD cycle_nub) {
    ...
    for (int i = 0; i != d; i++ ) {
        ...
        this->card[i]._ERROR = ""; // Initialize error property
    }

    // Create ManyInfo object to return
...
void ToHex(uint16_t cycle_nub, uint8_t &MSB, uint8_t &LSB) {
    // Extract the most significant byte (MSB)
    MSB = (cycle_nub >> 8) & 0xFF;
...
String UHF_RFID::Set_the_select_parameter_directive(String str_epc, UBYTE SelParam, UDOUBLE Ptr, UBYTE MaskLen, UBYTE Truncate)
{
  UBYTE b[8] = {0xBB, 0x01, 0x0C, 0x00, 0x01, 0x00, 0x0E, 0x7E};
   ...
   Serial.println(str_epc);
   Serial.println(SelParam);
   Serial.println(Ptr);
   Serial.println(MaskLen);
   Serial.println(Truncate);
...
Serial.println("********6*********");
  ...
  else
  {
    if (Verify_the_return(b, 6))
    {
      ...
      Serial.println("*******10*********");
      Cardinformation._Successful = "Success";
      Serial.println("********11*********");
      Serial.println("********12*********");
    }
    return Cardinformation;
